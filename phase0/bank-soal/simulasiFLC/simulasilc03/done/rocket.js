...
function nextTargetArea(code) {
  ...
  var sum =0
      if (codAv.length>3){
        for (let k=0; k<codAv.length-1; k++){
      sum += Number(codAv[k])
        }
      }
        else{
          for  (let i=0; i<codAv.length; i++){
          sum +=Number(codAv[i])
          }
    } 
   
  return "District "+ sum +" is the next target!"
  }
  ...
  ////4673
  ; // "District 17 is the next target!"
  console.log(nextTargetArea('EFNEXRSIVHUSEORTE')); // "District 20 is the next target!" 46373
